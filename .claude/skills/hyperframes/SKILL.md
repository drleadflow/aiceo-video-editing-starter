...
- [ ] `npx hyperframes lint` and `npx hyperframes check` both pass
...
`hyperframes check` runs a WCAG contrast audit by default. It seeks to 5 timestamps, screenshots the page, samples background pixels behind every text element, and computes contrast ratios. Failures appear as warnings:
...
Use `npm run check -- <composition-dir>` for the pinned renderer's runtime,
layout and motion checks. Inspect encoded transition frames and listen to audio.
The upstream animation-map and contrast-report scripts depended on an unbundled
producer package and are intentionally excluded from this starter.
