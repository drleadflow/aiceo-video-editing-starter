...
- If a render has black video layers, diagnose the encoded frames and renderer version before changing worker settings.

## 4 · Gates

The source project used bespoke QA scripts that are not bundled here. Use this
starter's executable checks and perform the visual checks below manually:

```sh
npm run check -- projects/<slug>/v01
node scripts/validate-beat-sync.mjs projects/<slug>/v01
```

Use the beat validator only for compositions with transcript-anchored beats.
Check legibility, continuous motion, dead frames, speaker visibility and scene
seams in the encoded export; record actual evidence in VERIFY.md.
